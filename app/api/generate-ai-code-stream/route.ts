...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
                ...
                const completionResult = await streamText({
                  model: completionClient(actualModel || model.replace('anthropic/', '').replace('openai/', '')),
                  messages: [
                    { 
                      role: 'system', 
        ...
        // Validate that generatedCode contains actual file content
        ...
        console.log(`[generate-ai-code-stream] Sending completion - ${fileCount} files, ${generatedCode.length} characters`);
        ...
        if ((error as any).message?.includes('tool call validation failed')) {
          ...
          // Send error event to frontend with detailed information
          const errorMessage = `Stream processing failed: ${(error as Error).message}`;
          console.error('[generate-ai-code-stream] Sending error to frontend:', errorMessage);
          ...
          await sendProgress({
            ...
            details: {
              ...
              isEdit,
              hasContext: !!context,
              stackTrace: (error as Error).stack?.substring(0, 500)
            }
          });
        }
      } finally {
